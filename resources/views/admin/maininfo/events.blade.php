...
<div class="events w-100">
	<div class="card m-4 p-3">
		...
		@endif
		<div>
			<button class="btn btn-primary add-btn">add event</button>
		</div>
		<form method="POST" action="{{url('admin/events')}}" enctype="multipart/form-data" class="border p-3 add-form">
			@csrf()
				...
				<tbody>
					@foreach($events as $event)
						<tr>
							...
							<td>
								<img src="{{url('upload/events',$event->image_name)}}" class="w-100">
							</td>
							<td class="text-center">
								<form method="GET" action="{{url('admin/events',$event->id)}}/edit">
